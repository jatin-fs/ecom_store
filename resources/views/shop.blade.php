                    ...
                    <div id="accordion-filter-1" class="accordion-collapse collapse show border-0" aria-labelledby="accordion-heading-1" data-bs-parent="#categories-list">
                        <div class="accordion-body px-0 pb-0 pt-3">
                            <ul class="list list-inline mb-0 category-list">
                                ...
                                <li class="list-item">
                                    <span class="menu-link py-1"> <input type="checkbox" name="category" value="{{$category->id}}" class="chk-category" @if(in_array($category->id,explode(',',$f_categories))) checked="checked" @endif /> {{$category->name}}</span>
                                    <span class="text-right float-right" style="float: right;">{{$category->products()->count()}}</span>
                                </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div id="accordion-filter-brand" class="accordion-collapse collapse show border-0" aria-labelledby="accordion-heading-brand" data-bs-parent="#brand-filters">
                        <div class="search-field multi-select accordion-body px-0 pb-0">
                            <ul class="list list-inline mb-0 brand-list">
                                ...
                                <li class="list-item">
                                    <span class="menu-link py-1"> <input type="checkbox" name="brands" value="{{$brand->id}}" class="chk-brand" @if(in_array($brand->id,explode(',',$f_brands))) checked="checked" @endif /> {{$brand->name}}</span>
                                    <span class="text-right float-right" style="float: right;">{{$brand->products()->count()}}</span>
                                </li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <div class="pc__info position-relative">
                            ...
                            <form method="POST" action="{{ route('wishlist.remove', ['rowId' => Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId ?? '']) }}">
                                ...
                                <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0 filled-heart" title="Remove from Wishlist">
                                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none" style="color:red;" xmlns="http://www.w3.org/2000/svg">
                                        <use href="#icon_heart" />
                                    </svg>
                                </button>
                            </form>
...
<form id="frmFilter" method="get" action="{{route('shop.index')}}">
    ...
    <input type="hidden" id="order" name="order" value="{{$sorting}}" />
...
<script>
    $(function() {
        ...
        $("input[type='checkbox'][name='brands']").on("change", function() {
            var brands = $("input[type='checkbox'][name='brands']:checked")
                .map(function() {
                    return $(this).val();
                }).get().join(",");
            $("#frmFilter input[id='hdnBrands']").val(brands);
            $("#frmFilter").submit();
        });


        $("input[type='checkbox'][name='category']").on("change", function() {
            var categories = $("input[type='checkbox'][name='category']:checked")
                .map(function() {
                    return $(this).val();
                }).get().join(",");
            $("#frmFilter input[id='hdnCategories']").val(categories);
            $("#frmFilter").submit();
        });

        $("[name='price_range']").on("change", function() {
            $("#frmFilter input[name='min']").val($(this).val().split(',')[0]);
            $("#frmFilter input[name='max']").val($(this).val().split(',')[1]);
            setTimeout(() => {
                $("#frmFilter").submit();
            }, 2000)
        });
